...
import { dashCardData, recentActivity, warningData } from "../components/Data";
        ...
        <div className=" bg-gray-100">
            <div className=" w-293 m-auto py-11">
                <div className=" grid grid-cols-5 gap-5.5">
                    {dashCardData?.map((dashc, i) => {
                        return (
                            <div className="bg-surface rounded-2xl p-5 shadow-sm border border-gray-200 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 text-primary text-20 font-regular" key={i}>
                                <p>{dashc?.title}</p>
                                <p>{dashc?.price}</p>
                            </div>
                        )
